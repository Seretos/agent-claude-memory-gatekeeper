...
// ---------------------------------------------------------------------------
// Unit tests — generateMemoryIndex (ticket #15)
// ---------------------------------------------------------------------------

console.log("\n--- Unit tests: generateMemoryIndex ---");

test("generateMemoryIndex: two .md files → MEMORY.md contains header and both entries sorted", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  fs.writeFileSync(path.join(tmpDir, "ZNote.md"), "z content");
  fs.writeFileSync(path.join(tmpDir, "Alpha.md"), "a content");

  generateMemoryIndex(tmpDir);

  const content = fs.readFileSync(path.join(tmpDir, "MEMORY.md"), "utf8");
  assert(content.includes("# Memory Index"), "header present");
  assert(content.includes("- [Alpha](Alpha.md)"), "Alpha entry present");
  assert(content.includes("- [ZNote](ZNote.md)"), "ZNote entry present");
  // Sorted: Alpha before ZNote
  assert(content.indexOf("Alpha") < content.indexOf("ZNote"), "entries sorted alphabetically");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: excludes MEMORY.md itself from the listing", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  fs.writeFileSync(path.join(tmpDir, "NOTE.md"), "content");
  fs.writeFileSync(path.join(tmpDir, "MEMORY.md"), "old index");

  generateMemoryIndex(tmpDir);

  const content = fs.readFileSync(path.join(tmpDir, "MEMORY.md"), "utf8");
  // MEMORY.md must not appear as a listing entry
  assert(!content.includes("- [MEMORY](MEMORY.md)"), "MEMORY.md excluded from listing");
  assert(content.includes("- [NOTE](NOTE.md)"), "NOTE.md still listed");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: empty dir (no .md files) → MEMORY.md written with header only, no list lines", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  ...
  assert(content.includes("# Memory Index"), "header present");
  // No list line
  assert(!content.includes("- ["), "no list entries for empty dir");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: single file → lists that one file", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  fs.writeFileSync(path.join(tmpDir, "Solo.md"), "solo content");

  generateMemoryIndex(tmpDir);

  const content = fs.readFileSync(path.join(tmpDir, "MEMORY.md"), "utf8");
  assert(content.includes("- [Solo](Solo.md)"), "single file listed");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: called twice → second call overwrites, listing reflects current state", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  fs.writeFileSync(path.join(tmpDir, "First.md"), "first");

  generateMemoryIndex(tmpDir);
  const contentAfterFirst = fs.readFileSync(path.join(tmpDir, "MEMORY.md"), "utf8");
  assert(contentAfterFirst.includes("- [First](First.md)"), "first call lists First.md");

  // Add a second file, regenerate.
  fs.writeFileSync(path.join(tmpDir, "Second.md"), "second");
  generateMemoryIndex(tmpDir);
  const contentAfterSecond = fs.readFileSync(path.join(tmpDir, "MEMORY.md"), "utf8");
  assert(contentAfterSecond.includes("- [First](First.md)"), "second call still lists First.md");
  assert(contentAfterSecond.includes("- [Second](Second.md)"), "second call lists Second.md");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: dir does not exist → does not throw, emits to stderr", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  ...
  process.stderr.write = (msg) => { stderrCaptured += msg; return true; };

  let threw = false;
  try {
    generateMemoryIndex(nonExistentDir);
  } catch {
    threw = true;
  } finally {
  ...
  assert(!threw, "generateMemoryIndex must not throw when dir absent");
  assert(stderrCaptured.length > 0, "error emitted to stderr");

  fs.rmSync(tmpDir, { recursive: true });
});

test("generateMemoryIndex: dir is a file (not a directory) → does not throw, emits to stderr", () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mem-gk-genidx-"));
  const fileNotDir = path.join(tmpDir, "i-am-a-file");
  fs.writeFileSync(fileNotDir, "I am a file");
  ...
  let threw = false;
  try {
    generateMemoryIndex(fileNotDir);
  } catch {
    threw = true;
  } finally {
    process.stderr.write = origWrite;
  ...
  assert(!threw, "generateMemoryIndex must not throw when path is a file");
  assert(stderrCaptured.length > 0, "error emitted to stderr");
...
// ---------------------------------------------------------------------------
// E2E regression tests — ticket #15: MEMORY.md index generation
// ---------------------------------------------------------------------------

console.log("\n--- E2E regression tests (ticket #15): MEMORY.md index generation ---");

// Regression: Write redirect → MEMORY.md exists and contains the written file's name.
test("Regression #15: Write redirect → gatekeeper MEMORY.md exists and lists the written file", () => {
  const { base, slug, liveFile } = makeTempBase("slug-idx1", "NOTE.md");

  const result = runHook(makeWriteEvent(liveFile, "hello memory"));
  assertEqual(result.status, 0, "exit 0");
  assertEqual(result.stderr.trim(), "", "no stderr errors");

  const gkMemDir = path.join(base, "gatekeeper", slug, "memory");
  const memoryMd = path.join(gkMemDir, "MEMORY.md");
  assert(fs.existsSync(memoryMd), "MEMORY.md exists in gatekeeper memory dir");

  const content = fs.readFileSync(memoryMd, "utf8");
  assert(content.includes("# Memory Index"), "MEMORY.md has header");
  assert(content.includes("- [NOTE](NOTE.md)"), "MEMORY.md lists NOTE.md");

  fs.rmSync(base, { recursive: true });
});

// Regression: Two Writes to different files → MEMORY.md lists both filenames.
test("Regression #15: Two Writes to different files → MEMORY.md lists both", () => {
  const { base, slug } = makeTempBase("slug-idx2", "NOTE.md");
  const liveFile1 = path.join(base, "projects", slug, "memory", "Alpha.md");
  const liveFile2 = path.join(base, "projects", slug, "memory", "Beta.md");

  runHook(makeWriteEvent(liveFile1, "alpha content"));
  runHook(makeWriteEvent(liveFile2, "beta content"));

  const memoryMd = path.join(base, "gatekeeper", slug, "memory", "MEMORY.md");
  assert(fs.existsSync(memoryMd), "MEMORY.md exists");

  const content = fs.readFileSync(memoryMd, "utf8");
  assert(content.includes("- [Alpha](Alpha.md)"), "Alpha.md listed");
  assert(content.includes("- [Beta](Beta.md)"), "Beta.md listed");

  fs.rmSync(base, { recursive: true });
});

// Regression: MEMORY.md must NOT be created under the live projects tree.
test("Regression #15: Write redirect → MEMORY.md NOT created under projects/<slug>/memory/", () => {
  const { base, slug, memoryDir, liveFile } = makeTempBase("slug-idx3", "NOTE.md");

  runHook(makeWriteEvent(liveFile, "content"));

  const liveMd = path.join(memoryDir, "MEMORY.md");
  assert(!fs.existsSync(liveMd), "MEMORY.md must NOT exist in live projects tree");

  fs.rmSync(base, { recursive: true });
});

// Regression: Edit seed-and-apply on a live-only file → gatekeeper MEMORY.md exists and lists the seeded file.
test("Regression #15: Edit seed-and-apply → gatekeeper MEMORY.md exists and lists the seeded file", () => {
  const { base, slug, memoryDir, liveFile } = makeTempBase("slug-idx4", "LIVE.md");
  fs.mkdirSync(memoryDir, { recursive: true });
  fs.writeFileSync(liveFile, "hello world\nsome content\n");
  ...
  const memoryMd = path.join(base, "gatekeeper", slug, "memory", "MEMORY.md");
  assert(fs.existsSync(memoryMd), "MEMORY.md exists after Edit seed-and-apply");

  const content = fs.readFileSync(memoryMd, "utf8");
  assert(content.includes("- [LIVE](LIVE.md)"), "MEMORY.md lists LIVE.md");

  fs.rmSync(base, { recursive: true });
});

// Regression: Bash delete-intent (rm) → MEMORY.md exists and lists the tombstone file.
test("Regression #15: Bash delete-intent (rm) → MEMORY.md exists in gatekeeper memory dir and lists tombstone", () => {
  const { base, slug, memoryDir, liveFile } = makeTempBase("slug-idx5", "NOTE.md");
  fs.mkdirSync(memoryDir, { recursive: true });
  fs.writeFileSync(liveFile, "live content");
  ...
  const gkMemDir = path.join(base, "gatekeeper", slug, "memory");
  const memoryMd = path.join(gkMemDir, "MEMORY.md");
  assert(fs.existsSync(memoryMd), "MEMORY.md exists after Bash delete-intent");

  const content = fs.readFileSync(memoryMd, "utf8");
  assert(content.includes("- [NOTE](NOTE.md)"), "MEMORY.md lists tombstone NOTE.md");

  fs.rmSync(base, { recursive: true });
});

// Edge case: fault-tolerance under FS error — MEMORY.md write blocked, hook exits 0, deny still emitted.
test("Fault-tolerance #15: generateMemoryIndex blocked by file-at-MEMORY.md-path → hook exits 0, deny still emitted, stderr contains error", () => {
  const { base, slug, memoryDir, liveFile } = makeTempBase("slug-idx-fault", "NOTE.md");

  // Pre-create a directory at the path where MEMORY.md would be written,
  // so writeFileSync(MEMORY.md) will fail (cannot write file over a directory).
  const gkMemDir = path.join(base, "gatekeeper", slug, "memory");
  fs.mkdirSync(gkMemDir, { recursive: true });
  // Create a directory named MEMORY.md to block the file write.
  const memoryMdPath = path.join(gkMemDir, "MEMORY.md");
  fs.mkdirSync(memoryMdPath, { recursive: true });

  const result = runHook(makeWriteEvent(liveFile, "some content"));
  assertEqual(result.status, 0, "exit 0 even when MEMORY.md write is blocked");

  // The deny must still be emitted.
  const out = JSON.parse(result.stdout.trim());
  assertEqual(out.hookSpecificOutput.permissionDecision, "deny", "deny still emitted");
